  ...
  methods: {
    videoStop() {
      ...
      if (!this.isGuest && !this.viewedVideo.includes(this.gamesStore.games[this.gameIndex]._id)) {
        ...
        if (this.currentVideoStop <= (this.gamesStore.games[this.gameIndex].videoStops.length - 1) && Math.trunc(this.player.currentTime()) >= this.gamesStore.games[this.gameIndex].videoStops[this.currentVideoStop].timestamp && !this.player.paused()) {
          this.player.pause()
