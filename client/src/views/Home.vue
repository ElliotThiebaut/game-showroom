  ...
  <div v-if="gamesStore.games.length !== 0 && !isLoading">
    ...
    <div class="my-10">
      ...
      <div class="w-full text-center mx-auto font-subtitle text-primary-red mt-4 text-lg">
        ...
        <h2 v-else>🎉 Congratulations 🎉<br> You've completed, all videos !</h2>
      </div>
    </div>

    <!-- Game grid -->
    <div v-if="!isLoading" class="grid grid-cols-1 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 2xl:grid-cols-4 2xl:gap-10">
      <GameCard @click="openGame(game._id)" v-for="game in gamesStore.games" :imgUrl="game.imgUrl" :category="game.category" :name="game.name" :id="game._id"/>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    endIntroVideo() {
      this.userStore.updateFirstVisit()
          .then(() => {
