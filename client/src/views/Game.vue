<template>
    <!-- Question modal -->
    <GameQuestionModal
        @close-modal="closeQuestionModal"
        :game-id="gamesStore.games[gameIndex]._id"
        :lastVideoStop="lastVideoStop"
        :questions="gamesStore.games[gameIndex].videoStops[videoStop].questions"
        v-if="questionModalVisibility"
    />
    ...
    <div v-if="!isLoading">
        ...
        <div class="max-w-3xl mx-auto lg:py-8 lg:pt-0">
            ...
            <video-player
                @last-video-stop="showLastQuestionModal"
                @video-stop="showQuestionModal"
                @video-resumed="videoResumed"
            ...
            <div class="font-subtitle text-lg lg:text-xl my-5 lg:my-10 mb-10">
                <div class="flex flex-col-reverse lg:flex-row justify-between lg:mb-10">
                    <div>
                        <p>
                            Designed by <span class="font-bold text-primary-red">{{ gamesStore.games[gameIndex].designer.join(", ") }}</span>
                        </p>
                        <p>
                            Illustrated by by <span class="font-bold text-primary-red">{{ gamesStore.games[gameIndex].illustrator.join(", ") }}</span>
                        </p>
                    </div>
                    <div v-if="!isViewed" class="flex items-center w-full lg:w-1/3 my-5 lg:my-0">
                        <div @click="launchAllQuestions" class="text-white py-3 rounded cursor-pointer font-subtitle bg-primary-red shadow-lg cursor-pointer w-full text-center">
...
export default {
    ...
    methods: {
        ...
        deleteVideo() {
            this.deleteViewedVideo(this.gamesStore.games[this.gameIndex]._id).catch(e => {
