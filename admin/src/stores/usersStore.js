...
export const useUsersStore = defineStore("users", {
    ...
    actions: {
        async exportUser() {
            ...
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_URL}/users?export=true`, {
                    responseType: "blob",
                    headers: {
        ...
        async deleteUser(email) {
            ...
            try {
                const response = await axios.delete(`${import.meta.env.VITE_API_URL}/user/${this.users[this.users.findIndex(user => user.email === email)]._id}`, {
                    headers: {
                        Authorization: `KEY ${logicStore.key}`,
                    },
                });
                ...
                this.users.splice(
                    this.users.findIndex(user => user.email === email),
                    1,
                );
            } catch (error) {
                throw new Error(error.response.status.toString());
