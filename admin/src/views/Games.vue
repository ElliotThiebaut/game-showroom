  ...
  <div v-if="isDeletingGameModal">
    <div class="backdrop-blur-sm bg-gray-600/50 overflow-auto py-10 lg:px-20 xl:px-40 2xl:px-72 3xl:px-[35rem] px-5 fixed z-10 inset-0 w-full h-screen">
      <div class="bg-primary-red text-white text-center rounded-md px-5 lg:px-10 py-10 mt-44 max-w-lg mx-auto">
        ...
        <p class="my-10 font-text text-lg">All the answers related to this game will still be accessible.<br> To restore the game later, please contact the administrator.</p>
   ...
   <HeaderCard>
     <div>
       <p>Total number of games : <span class="font-bold text-lg lg:text-xl font-title">{{ gamesStore.getNumberOfGames }}</span></p>
     </div>
     <div @click="openEditor(-1)" class="w-full text-center mt-10 lg:mt-0 lg:w-auto bg-white rounded font-subtitle text-lg text-primary-red py-2 lg:px-5 flex justify-center items-center cursor-pointer">
       <p>Add a game</p>
     </div>
   </HeaderCard>

  <div class="mt-10 mb-16 flex-col">
    <GameLine class="my-5 py-3 border-b-2 first:pt-0 last:pb-0 last:border-none" @deleteGame="deleteGameModal" @openGameEditor="openEditor" v-for="game in gamesStore.games" :key="game._id" :game-index="gamesStore.getIndexOfGame(game._id)"  />
  </div>
...
export default {
  ...
  methods: {
    ...
    deleteGame() {
      this.gamesStore.deleteGame(this.gamesStore.games[this.deletingGameIndex]._id)
          .then(() => {
