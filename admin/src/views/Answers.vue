<template>
  <HeaderCard>
    <div>
      <p>Total number of answers : <span class="font-bold text-lg lg:text-xl font-title">{{ answersStore.getNumberOfAnswers }}</span></p>
      <p>Number of full answers: <span class="font-bold text-lg lg:text-xl font-title">{{ answersStore.getNumberOfFullAnswers }}</span></p>
    </div>
    <div @click="answersStore.exportAnswer" class="w-full text-center mt-10 lg:mt-0 lg:w-auto bg-white rounded font-subtitle text-lg text-primary-red py-2 lg:px-5 flex justify-center items-center cursor-pointer">
      <p>Export</p>
    </div>
  </HeaderCard>
...
export default {
  ...
  created() {
    if (this.answersStore.answers.length === 0 && this.logicStore.key) {
      this.isLoading = true
      this.answersStore.fetchAnswers()
          .then(() => {
