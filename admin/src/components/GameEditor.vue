<template>
    <div class="backdrop-blur-sm bg-gray-600/50 overflow-auto py-10 lg:px-20 xl:px-40 2xl:px-72 px-5 fixed z-10 inset-0 w-full h-screen">
        <div class="bg-white text-center rounded-md px-5 lg:px-10 py-10 mx-auto">
            <h1 v-if="!isNewGame" class="font-title text-xl text-primary-red mb-5 xl:mb-10 xl:text-2xl" :class="isGameIncomplete ? 'text-yellow-500 mb-1  xl:mb-3' : ''">
                Editing : {{ formGame.name }} <span v-if="isGameIncomplete">(Inactive)</span>
            </h1>
            ...
            <form v-if="!firstFileUpload" @submit.prevent="saveGame" method="post">
                <div class="flex flex-col xl:flex-row xl:justify-between">
                    <div class="w-full xl:mr-5 xl:w-3/5">
                        ...
                        <form v-if="!isNewGame" @submit.prevent="saveCover" class="flex flex-col text-left mb-8" method="post" name="cover">
                            ...
                            <input
                                ref="coverFile"
                                class="file:bg-primary-red/10 hover:file:bg-primary-red/20 file:text-primary-red file:font-subtitle file:rounded-lg file:border-none file:p-2 file:mr-4 mt-3 bg-primary-red/5 rounded-lg font-text cursor-pointer file:cursor-pointer"
                                id="cover"
                                type="file"
                            />
                            <button
                                ...
                                type="submit"
                                class="bg-primary-red rounded text-white font-title py-2 px-4 mt-5 flex justify-center"
                                :class="uploadingVideo ? 'cursor-not-allowed bg-primary-red/50' : ''"
                            >
                        ...
                        <form v-if="!isNewGame" @submit.prevent="saveVideo" class="flex flex-col text-left mb-8" enctype="multipart/form-data" method="post" name="cover">
                            ...
                            <button
                                ...
                                type="submit"
                                class="bg-primary-red rounded text-white font-title py-2 px-4 mt-5 flex justify-center"
                                :class="uploadingCover ? 'cursor-not-allowed bg-primary-red/50' : ''"
                            >
                ...
                <div class="flex justify-between w-full mt-10">
                    <button
                        :disabled="uploadingCover || uploadingVideo"
                        type="submit"
                        class="mx-auto bg-primary-red rounded text-white font-title py-2 px-4 w-2/5"
                        :class="uploadingVideo || uploadingCover ? 'cursor-not-allowed bg-primary-red/50' : ''"
                    >
                        Save
                    </button>
                    <button
                        :disabled="uploadingCover || uploadingVideo"
                        @click="$emit('cancelEditor')"
                        class="mx-auto border-2 border-primary-red text-primary-red font-title py-2 px-4 rounded w-2/5"
                        :class="uploadingVideo || uploadingCover ? 'cursor-not-allowed border-primary-red/50 text-primary-red/50' : ''"
                    >
                        Cancel
                    </button>
                </div>
                ...
                <div class="flex flex-col lg:flex-row lg:w-full lg:justify-around lg:mt-24">
                    <form @submit.prevent="saveCover" class="flex flex-col text-left mb-14" method="post" name="cover">
                        ...
                        <button
                            ...
                            type="submit"
                            class="bg-primary-red rounded text-white font-title py-2 px-4 mt-5 flex justify-center"
                            :class="uploadingVideo || coverButton === 'Cover updated !' ? 'cursor-not-allowed bg-primary-red/50' : ''"
                        >
                            <svg v-if="uploadingCover" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    ...
                    <form @submit.prevent="saveVideo" class="flex flex-col text-left mb-8" enctype="multipart/form-data" method="post" name="cover">
                        ...
                        <button
                            ...
                            type="submit"
                            class="bg-primary-red rounded text-white font-title py-2 px-4 mt-5 flex justify-center"
                            :class="uploadingCover || videoButton === 'Video updated !' ? 'cursor-not-allowed bg-primary-red/50' : ''"
                        >
                            <svg v-if="uploadingVideo" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
...
export default {
    ...
    methods: {
        ...
        saveGame() {
            this.formGame.designer = this.formGame.designer
                .toString()
                .split(",")
                .map(el => el.trim());
            this.formGame.illustrator = this.formGame.illustrator
                .toString()
                .split(",")
                .map(el => el.trim());
